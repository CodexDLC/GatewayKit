...
from .dto import (
    APIResponse, ApiLoginRequest, ApiLoginResponse,
    ApiRegisterRequest, ApiRegisterResponse, ApiValidateResponse,
    ApiRefreshTokenRequest, ApiRefreshTokenResponse,
    ApiLogoutRequest, ApiLogoutResponse
)
...
async def register(
    ...
    if not rpc_resp or not rpc_resp.get("success"):
        error_code = rpc_resp.get("error_code", ErrorCode.AUTH_USER_EXISTS) if rpc_resp else ErrorCode.RPC_TIMEOUT
        raise HTTPException(
            status_code=get_http_status(error_code),
            detail=rpc_resp.get("message", "Registration failed.") if rpc_resp else "Auth service timeout."
        )
...
async def validate_token(
    ...
    if not rpc_resp or not rpc_resp.get("valid"):
        raise HTTPException(status_code=401, detail=rpc_resp.get("error_message",
                                                                 "Invalid token") if rpc_resp else "Auth service timeout.")
